...
import { fetchSalaryIntelligence, fetchTopPaying, fetchCategories } from "../../services/intelligenceService";
...
export default function SalaryInsightsPage() {
  ...
  const loadTopPaying = async (cat) => {
    setTopLoading(true);
    try {
      const res = await fetchTopPaying({ category: cat || undefined, limit: 15 });
      setTopPaying(res.data || []);
    } catch { setTopPaying([]); }
  ...
  const handleRoleSelect = async (role) => {
    ...
    try {
      const res = await fetchSalaryIntelligence(role.slug);
  ...
  return (
    <div className="intel-page">
      <div className="intel-page__header">
        ...
        <p className="intel-page__subtitle">UK salary benchmarks, ranges, and career progression data.</p>
      </div>

      {/* Search for specific role */}
      <div className="intel-page__search">
        <RoleSearch onSelect={handleRoleSelect} placeholder="Search a role for salary details..." />
      </div>
      ...
      {/* Top paying table */}
      <div className="intel-page__result" style={{ marginTop: "var(--space-8)" }}>
        <div className="intel-top-header">
          <h3 className="intel-label">Top Paying Roles</h3>
          <div className="intel-top-filters">
            <button
              className={`intel-filter-chip ${!filterCat ? "intel-filter-chip--active" : ""}`}
              onClick={() => handleCatFilter("")}
            >All</button>
            {categories.slice(0, 8).map((cat) => (
              <button
                key={cat}
                className={`intel-filter-chip ${filterCat === cat ? "intel-filter-chip--active" : ""}`}
        ...
        {topLoading ? (
          <div className="intel-table-skeleton">
            {[1,2,3,4,5].map(i => <div key={i} className="skel skel--lg" style={{ width: "100%", height: 40, marginTop: 4 }} />)}
          </div>
        ) : topPaying.length > 0 ? (
          <div className="intel-table">
            <div className="intel-table__head">
              <span className="intel-table__th intel-table__th--wide">Role</span>
              <span className="intel-table__th">Category</span>
              <span className="intel-table__th">Seniority</span>
              <span className="intel-table__th intel-table__th--right">Avg Salary</span>
              <span className="intel-table__th intel-table__th--right">Range</span>
            </div>
            {topPaying.map((role, i) => (
              <button key={role.slug} className="intel-table__row" onClick={() => handleRoleSelect(role)}>
                <span className="intel-table__td intel-table__td--wide">
                  <span className="intel-table__rank">{i + 1}</span>
                  {role.title}
                </span>
                <span className="intel-table__td">{role.category}</span>
                <span className="intel-table__td">{role.seniority}</span>
          ...
          <div className="dash-empty dash-empty--compact">
            <p className="dash-empty__text">No roles found for this filter</p>
          </div>
        )}
      </div>
    </div>
