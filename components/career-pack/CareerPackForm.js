...
//
// CHANGED: accepts an `initialValues` prop so the Career Pack page can
// pre-populate the form fields when the user arrives from an EDGEX action
// chip (e.g. "Get full Career Pack" navigates to /career-pack?role=X&target=Y).
// ============================================================================

import { useState, useEffect } from "react";
import RoleSearch from "../intelligence/RoleSearch";

export default function CareerPackForm({ onSubmit, loading, initialValues }) {
  // Pre-populate from initialValues when provided (EDGEX context injection)
  const [from,     setFrom]     = useState(null);
  const [to,       setTo]       = useState(null);
  const [skills,   setSkills]   = useState("");
  const [yearsExp, setYearsExp] = useState("");

  // Apply initialValues on mount (or when they change — e.g. query params arrive
  // after router.isReady)
  useEffect(() => {
    if (!initialValues) return;
    if (initialValues.role   && !from)   setFrom({ slug: initialValues.role,   title: _slugToTitle(initialValues.role)   });
    if (initialValues.target && !to)     setTo  ({ slug: initialValues.target, title: _slugToTitle(initialValues.target) });
    if (initialValues.skills && !skills) setSkills(
      Array.isArray(initialValues.skills)
        ? initialValues.skills.join(", ")
        : initialValues.skills
    );
    if (initialValues.yearsExp && !yearsExp) setYearsExp(String(initialValues.yearsExp));
  }, [initialValues]);
    ...
    <form className="pack-form" onSubmit={handleSubmit}>
      <div className="pack-form__hero">
        ...
        <p className="pack-form__desc">
          One comprehensive report combining roadmap, skills gap, resume blueprint,
          LinkedIn optimisation, interview prep, salary intel, and visa assessment.
        </p>
      </div>
      ...
      <div className="pack-form__fields">
        ...
        <div className="pack-form__field">
          ...
          <RoleSearch
            ...
            onSelect={(r) => setFrom(r)}
            initialValue={from?.title || ""}
          />
          {from && (
            <span className="pack-form__selected">
              {from.title || from.slug || from}
            </span>
          )}
        </div>
